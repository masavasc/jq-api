...
export async function GET(req: NextRequest) {
  try {
    ...
    const minScore = Number(process.env.ALERT_MIN_SCORE || 8);
    const results: any[] = [];
    ...
    for (const t4 of t4s) {
      ...
      const q = (px.daily_quotes || []).sort((a: any, b: any) =>
      ...
      const rsiPrev = rsi(close.slice(0, close.length - 1), 14);
      ...
      const dipCount = [near50, drawdown, atrOk].filter(Boolean).length;

      // 反発（confirm）
      const confirmBreak = c0 > h1;
      const confirmVol = vol20 ? vol0 >= 1.2 * vol20 : false;
      const confirmRsi = rsi14 ? rsi14 >= 45 : false;
      const confirmRsiUp = rsi14 && rsiPrev ? rsi14 > rsiPrev : false;
      ...
      if (dipCount >= 2) score += 2;
      if (dipCount === 3) score += 1;

      if (confirmBreak) score += 1;

      if (vol20) {
        if (vol0 >= 1.5 * vol20) score += 2;
        else if (confirmVol) score += 1;
      }

      if (rsi14) {
        if (rsi14 >= 50) score += 2;
        else if (confirmRsi) score += 1;
      }

      if (confirmRsiUp) score += 1;

      // 深すぎる下落は減点（大損予防）
      if (dd < -0.15) score -= 2;

      const signal = score >= minScore ? "BUY" : "NONE";
      ...
      results.push({
        ...
        vol20,
        drawdown20: dd,
        flags: { regime, dipCount, confirmBreak, confirmVol, confirmRsi, confirmRsiUp },
      });
    }
